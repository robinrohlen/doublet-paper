function sig = notchsignals(signal,fsamp)

frad = round(4/(fsamp/size(signal,2))); 
sig=zeros(size(signal));

for i=1:size(signal,1)                                    
    sig(i,:) = real(removelineinter(signal(i,:), frad, fsamp));  
end

function sig = removelineinter(signal,frad,fsamp)
fsignal = fft(signal);
...
sig = signal-ifft(fcorrec(1:length(fsignal)));
