...
load('data/sub-13/emg/sub-13_task-triangular30percentmvc_run-01_emg.mat')
...
xlim([5 40])
ylim([0 size(edition.Distimeclean{i},2)+2]);
yticks([0:10:30]);
ylim_tmp=get(gca,'ylim');

force_sig=abs(filtfilt(b,a,signal.ref_signal));
hold on;
...
cmap=turbo(size(edition.Distimeclean{1},2));
...
for i=1:length(indx)
    ...
    plot_broken_axis(locs_new(1:end-1)/(40e3),1000./(diff(locs_new)/(40)),50,[20 100],[100 200],20,[5 40],cmap(ind,:),0,0,0);
    hold off;
    set(gca,'TickDir','out');
    set(gcf,'color','w');
    set(gca,'FontSize',20);
